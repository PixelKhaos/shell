...
Item {
    ...
    readonly property bool hasImage: {
        if (!currentItem?.modelData || imageLoadError) return false;
    ...
    readonly property string imageSource: {
        if (!currentItem?.modelData) return "";
        const data = currentItem.modelData;
        
        if (data.isImage === true && imageDataUrl !== "") return imageDataUrl;
        if (data.hasImageUrl === true) return extractedImageUrl || data.imageUrl || "";
    ...
    function decodeImageToDataUrl(): void {
        if (!currentItem?.modelData || currentItem.modelData.isImage !== true) return;
    ...
    function decodeHtmlForImageUrl(): void {
        if (!currentItem?.modelData || currentItem.modelData.needsDecodeForUrl !== true) return;
    ...
    Process {
        ...
        onExited: {
            if (root.currentItem?.modelData?.isImage === true) {
                ...
                if (b64) root.imageDataUrl = "data:image/png;base64," + b64;
    ...
    Process {
        ...
        onExited: {
            ...
            if (root.currentItem?.modelData?.needsDecodeForUrl === true) {
                ...
                if (srcMatch?.[1]) root.extractedImageUrl = srcMatch[1];
            }
        }
    }
    ...
    readonly property real targetHeight: {
        if (!shouldShow || !hasImage) return 0;
    ...
    IconButton {
        ...
        visible: {
            if (!root.currentItem?.modelData) return false;
